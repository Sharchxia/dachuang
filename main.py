from ports import port_init,tx,rx
# print('aa55000100000a0d'.encode('utf-8'))
# print(('0xaa55000100000a0d'.encode('utf-8').hex()))
# print(('aa55000100000a0d'.encode('utf-8').hex()))
# print(('0xa'.encode('utf-8').hex()))
# print(('a'.encode('utf-8').hex()))
# print(b'aa55000100000a0d')
# print(type(b'\xaa'.hex()))
# print(b'\xaa')
# print(b'aa')
# print(type(b'\xaa'))
# print(type(b'hello'))
# print(b'hello')
#
# print((b'\xaa55000100000a0d'.hex()))
# print((b'aa55000100000a0d'.hex()))
#
# num = '{:x}'.format(100)
# print(num.encode('utf-8'))
# print(num.encode('utf-8').hex())
# sd = convert_float32_to_bytes(123.12)
# print('sd: ',sd,type(sd),sd.hex(),str(sd.hex()))
# print(convert_bytes_to_string(sd))
# print(convert_string_to_bytes('1234'))
# print(str(port_list()[0]))
# print(port_set('COM3'))
# print('angle_euler:',angle_euler())
# print('angle_speed:',angle_speed())
# print('angle_acc:',angle_acc())
# print('light:',light_status())
# print('source:',source_storage())
# print('temper:',temper_status())
# print(struct.unpack('f','7e59af43'))
c = 'addfafga'.encode('utf-8')
print(c)
print(c.decode('utf-8'))

# print(port_init.port_list())
# port_init.port_set('123')
# port_init.baud_set(123)
# rx.angle_acc()
# rx.angle_speed()
# rx.angle_euler()
# rx.light_status()
# rx.source_storage()
# rx.temper_status()
# rx.communicate()
# tx.close_all()
# tx.source_control()
# tx.wheel_control()
# tx.temp_control()
# tx.magnetic_control()
# tx.magnetic_magnitude()
# tx.wheel_speed_control()
# tx.temp_set()
# tx.temp_corresponding()
# tx.heat_corresponding()
tx.steering_angle()
# tx.communicate(12,'123')
